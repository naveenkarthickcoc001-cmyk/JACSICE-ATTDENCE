...
function logout() {
  // Sign out from Firebase Auth (if loaded)
  if (window._fb && typeof window._fb.signOut === 'function') {
    window._fb.signOut().catch(() => {});
  }
  // Legacy Supabase sign-out (kept for backward compatibility)
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Adjust sidebar for student role globally
  if (isStudent()) {
    const allowedHrefs = ['dashboard.html', 'marks.html', 'reports.html', 'schedule.html', 'settings.html'];
    document.querySelectorAll('.sidebar-nav .nav-item').forEach(item => {
      const href = item.getAttribute('href');
      // Hide if it has an href and is not in the allowed list
      if (href && !allowedHrefs.includes(href)) {
        item.style.display = 'none';
      }
    });

    // Hide any nav-section that has all its nav-items hidden
    document.querySelectorAll('.sidebar-nav .nav-section').forEach(sec => {
      const items = Array.from(sec.querySelectorAll('.nav-item'));
      if (items.length > 0 && items.every(item => item.style.display === 'none')) {
