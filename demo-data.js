...
// ─── Supabase Database Fetch Helper ──────────────────────
async function fetchAllFromSupabase() {
  if (typeof supabaseClient === 'undefined' || supabaseClient === null) return;
  console.log("Fetching latest data from Supabase...");

  try {
    const fetchTable = async (table, setLocalArray, storageKey) => {
      const { data, error } = await supabaseClient.from(table).select('*');
      if (!error && data && data.length > 0) {
        setLocalArray(data);
        localStorage.setItem(`sa_${storageKey}`, JSON.stringify(data));
        console.log(`Fetched ${data.length} records from ${table}`);
      }
    };

    await Promise.all([
      fetchTable('departments', data => DEMO_DEPARTMENTS = data, 'departments'),
      fetchTable('sections', data => DEMO_SECTIONS = data, 'sections'),
      fetchTable('subjects', data => DEMO_SUBJECTS = data, 'subjects'),
      fetchTable('users', data => DEMO_USERS = data, 'users'),
      fetchTable('pending', data => DEMO_PENDING = data, 'pending'),
      fetchTable('marks', data => DEMO_MARKS = data, 'marks'),
      fetchTable('attendance', data => DEMO_ATTENDANCE = data, 'attendance'),
      fetchTable('att_summary', data => DEMO_ATT_SUMMARY = data, 'att_summary')
    ]);
